...
AS
 ...
 PROCEDURE UPDATE_LOAN_SUMMARY(officeID NUMBER, businessDate DATE);
END DAY_END_PROCESS;
...
IS
BEGIN
	WRITE_LOG('UPDATE_DAILY_CHARGE SP CALLED FROM ADJUSTMENT SP');
END;
...
BEGIN
	...
	INTO insDate 
	FROM DAILY_LOAN_TRX WHERE OFFICE_ID=6
	...
	INSERT INTO DAILY_LOAN_TRX(LOAN_SUMMARY_ID,OFFICE_ID,CENTER_ID,PRODUCT_ID,
		...
		FROM DAILY_LOAN_TRX dlt
		 WHERE dlt.OFFICE_ID=6-- And MemberID=70
		...
		AND l.OFFICE_ID=6 AND (CASE WHEN (l.PRINCIPAL_LOAN-l.LOAN_REPAID-e.LOAN_PAID)<0 THEN
			(l.PRINCIPAL_LOAN-l.LOAN_REPAID-e.LOAN_PAID)
		else 0 end)<>0;
	...
	INSERT INTO DAILY_LOAN_TRX(LOAN_SUMMARY_ID,OFFICE_ID,CENTER_ID,PRODUCT_ID,
		...
		FROM DAILY_LOAN_TRX dlt where dlt.OFFICE_ID=6
...
--Over LoanPaid from Principal Amount--
	INSERT INTO DAILY_LOAN_TRX(LOAN_SUMMARY_ID,OFFICE_ID,CENTER_ID,PRODUCT_ID,
		...
		TO_DATE('2019-10-03','YYYY-MM-DD') as TRX_DATE,r.INSTALLMENT_NO,
		TO_DATE('2019-10-03','YYYY-MM-DD') as INSTALLMENT_DATE,
		...
		WHERE ls.OFFICE_ID=6 AND SUBSTR(p.PRODUCT_CODE,0,1)<>'7' AND ls.IS_ACTIVE=1 
...
--		Exec LoanBalanceTranserToSavings @officeId,@InsDate,@CreateUser,@OrgID
...
PROCEDURE UPDATE_LOAN_SUMMARY(officeID IN NUMBER, businessDate IN DATE) 
IS
ex EXCEPTION;
PRAGMA exception_init(ex,-20001);
spName VARCHAR(80) := SYSDATE || ' [UPDATE_LOAN_SUMMARY]:';
