...
AS
 ...
 PROCEDURE UPDATE_LOAN_TRX_DUE(officeID NUMBER, orgID NUMBER, businessDate DATE);
 ...
 PROCEDURE UPDATE_COLLECTION_SAVING_SUMMARY(officeID NUMBER);
 PROCEDURE UPDATE_LOAN_SUMMARY_CHARGE_DUE_PAID_AMOUNT(officeID NUMBER, businessDate DATE);
END DAY_END_PROCESS;
...
ex EXCEPTION;
PRAGMA exception_init(ex,-20001);
spName VARCHAR(80) := SYSDATE || ' [UPDATE_COLLECTION_SAVING_SUMMARY]:';
firstDate DATE;
BEGIN
	UPDATE AUTO_VOUCHER_CHECK
		SET CHECK_AUTO_VOUCHER=1 WHERE  OFFICE_ID=officeID;
	
	Insert into COLLECTION_LOAN_SAVINGS_HISTORY
		( SUMMARY_ID, OFFICE_ID, CENTER_ID, MEMBER_ID, PRODUCT_ID, PRODUCT_CODE, 
		LOAN_PAID, DEPOSIT, WITHDRAWAL, COLLECTION_DATE, CREATE_USER, 
		CREATE_DATE, IS_UPLOADED, LOAN_DUE, INT_DUE, INSTALLMENT_NO, 
		PRINCIPAL_LOAN, LOAN_REPAID, INT_CHARGE, INT_PAID, BALANCE)
		SELECT  SUMMARY_ID, OFFICE_ID, CENTER_ID, MEMBER_ID, PRODUCT_ID, PRODUCT_CODE, 
		LOAN_PAID, DEPOSIT, WITHDRAWAL, COLLECTION_DATE, CREATE_USER, 
		CREATE_DATE, IS_UPLOADED, LOAN_DUE, INT_DUE, INSTALLMENT_NO, 
		PRINCIPAL_LOAN, LOAN_REPAID, INT_CHARGE, INT_PAID, BALANCE 
		FROM COLLECTION_LOAN_SAVINGS 
		WHERE OFFICE_ID=officeID;
...
PROCEDURE UPDATE_LOAN_SUMMARY_CHARGE_DUE_PAID_AMOUNT(officeID IN NUMBER, businessDate IN DATE) 
IS
ex EXCEPTION;
PRAGMA exception_init(ex,-20001);
spName VARCHAR(80) := SYSDATE || ' [UPDATE_LOAN_SUMMARY]:';
