...
BEGIN
	...
	DELETE FROM NEW_AC_IN_7 WHERE OFFICE_ID = officeID;
	DELETE FROM SAVING_TRANSACTION WHERE OFFICE_ID = officeID;
	DELETE FROM TEMP_SAVINGS_REGISTER WHERE OFFICE_ID = officeID;
	DELETE FROM TEMP_MONTHLY_PROCESS_1 WHERE OFFICE_ID = officeID;
	DELETE FROM TEMP_MONTHLY_PROCESS_2 WHERE OFFICE_ID = officeID;
	DELETE FROM DAILY_SAVING_COLLECTION WHERE OFFICE_ID = officeID;
	...
	Where (SAVING_TRX.OFFICE_ID = officeID And SAVING_TRX.ORGANIZATION_ID=orgID) 
		And (UTIL.GET_MONTH_FROM_DATE(SAVING_TRX.TRANSACTION_DATE) = cMon) 
		And (UTIL.GET_YEAR_FROM_DATE(SAVING_TRX.TRANSACTION_DATE) = cYear)
	...
	ON st.OFFICE_ID = ss.OFFICE_ID 
		...
		WHERE  st.OFFICE_ID = officeID AND st.ORGANIZATION_ID=orgID
		And (UTIL.GET_MONTH_FROM_DATE(st.TRANSACTION_DATE) = cMon) 
		And (UTIL.GET_YEAR_FROM_DATE(st.TRANSACTION_DATE) = cYear)
	...
	SELECT tsr.SAVING_SUMMARY_ID,tsr.ORGANIZATION_ID,tsr.OFFICE_ID,tsr.CENTER_ID, 
		...
		WHERE (((tsr.OFFICE_ID)=officeID) 
		AND ((tsr.BALANCE)>(tsr.BALANCE-tsr.PERSONAL_SAVINGS+tsr.WITHDRAWAL)) 
		AND ((UTIL.GET_DAY_FROM_DATE(TRANSACTION_DATE))>7) 
		AND ((UTIL.GET_MONTH_FROM_DATE(TRANSACTION_DATE))=cMon) 
		AND ((UTIL.GET_YEAR_FROM_DATE(TRANSACTION_DATE))=cYear))
		AND tsr.ORGANIZATION_ID=orgID;
	...
	SELECT t2.SAVING_SUMMARY_ID,t2.ORGANIZATION_ID,t2.OFFICE_ID, t2.CENTER_ID, 
		...
		vMonthStartDate AS INSTALLMENT_DATE,cMon TRANSACTION_MONTH,
		cYear TRANSACTION_YEAR, t2.NO_OF_ACCOUNT  
	...
	WHERE t2.OFFICE_ID=officeID AND t2.ORGANIZATION_ID=orgID
	...
	WHERE st.OFFICE_ID=officeID AND st.ORGANIZATION_ID=orgID AND ss.SAVING_STATUS = 1
	...
	ss.OFFICE_ID, ss.CENTER_ID, ss.MEMBER_ID, 
		...
		vMonthStartDate AS TRANSACTION_DATE, ss.NO_OF_ACCOUNT
	...
	AND ss.ORGANIZATION_ID=orgID AND dst.OFFICE_ID IS NULL
    ...
    Where st.OFFICE_ID = officeID AND st.ORGANIZATION_ID=orgID
		...
		AND UTIL.GET_MONTH_FROM_DATE(ss.OPENING_DATE) = cMon
		AND UTIL.GET_YEAR_FROM_DATE(ss.OPENING_DATE) = cYear
		AND ss.SAVING_STATUS = 1;
	...
	WHERE dst.OFFICE_ID = officeID) s
	...
	DELETE FROM MONTH_WISE_SAVING_INTEREST 
		WHERE OFFICE_ID = officeID AND UTIL.GET_MONTH_FROM_DATE(TRANSACTION_DATE)=cMon 
		AND UTIL.GET_YEAR_FROM_DATE(TRANSACTION_DATE)=cYear AND ORGANIZATION_ID=orgID;
	...
	On (ss.MEMBER_ID = dst.MEMBER_ID) 
		...
		AND ss.NO_OF_ACCOUNT = dst.NO_OF_ACCOUNT
		AND ss.SAVING_SUMMARY_ID = dst.SAVING_SUMMARY_ID  
	WHERE ss.SAVING_STATUS = 1 AND ss.OFFICE_ID = officeID;
	...
	UPDATE PROCESS_INFO 
		...
		WHERE  OFFICE_ID = officeID AND BUSINESS_DATE=monthEndDate;
	...
	(CASE WHEN p.INTEREST_CALCULATION_METHOD='F' THEN ul.INT_CHARGE
		...
		(case when ul.INSTALLMENT_NO>=ul.DURATION then ((ul.INTEREST_RATE/100)*UTIL.GET_DAY_DIFF(monthEndDate,ul.INSTALLMENT_DATE))/365 else (ul.INTEREST_RATE/100)/ul.DURATION end)
		WHEN p.INTEREST_CALCULATION_METHOD='D' THEN 
			ul.INT_CHARGE+(ul.PRINCIPAL_LOAN-ul.LOAN_REPAID)*ul.INTEREST_RATE* UTIL.GET_DAY_DIFF(monthEndDate,ul.INSTALLMENT_DATE) /36500 
	END) CURRENT_CHARGE,
	monthEndDate INSTALLMENT_DATE,
						ul.INSTALLMENT_NO+1 WEEK_PASSED
	 FROM LOAN_SUMMARY ul 
	INNER JOIN PRODUCTS p ON ul.PRODUCT_ID=p.PRODUCT_ID
	WHERE ul.OFFICE_ID=officeID AND ul.LOAN_STATUS=1
	AND ul.DISBURSE_DATE < vMonthStartDate --And ul.WeekPassed<ul.Duration
	AND p.PAYMENT_FREQUENCY='M'
	AND ul.LOAN_SUMMARY_ID NOT IN
		(
		SELECT LOAN_SUMMARY_ID
		FROM LOAN_TRX ul 
		WHERE ul.OFFICE_ID=officeID 
		AND ul.INSTALLMENT_DATE BETWEEN vMonthStartDate 
				AND monthEndDate
		)
	) s
	...
	AND t.INSTALLMENT_START_DATE <= monthEndDate)
	WHEN MATCHED THEN
	...
	WHEN p.INTEREST_CALCULATION_METHOD IN('A','E','R') THEN  (ul.PRINCIPAL_LOAN - ul.LOAN_REPAID) * 
		(CASE WHEN ul.INSTALLMENT_NO>=ul.DURATION THEN 
			((ul.INTEREST_RATE/100)*UTIL.GET_DAY_DIFF(monthEndDate,ul.INSTALLMENT_DATE))/365 
				ELSE (ul.INTEREST_RATE/100)/ul.DURATION END)
	WHEN p.INTEREST_CALCULATION_METHOD='D' THEN (ul.PRINCIPAL_LOAN-ul.LOAN_REPAID)*ul.INTEREST_RATE* UTIL.GET_DAY_DIFF(monthEndDate,ul.INSTALLMENT_DATE) /36500 
	END) INT_CHARGE,0 INT_PAID,monthEndDate INSTALLMENT_DATE,10 TRX_TYPE,ul.INSTALLMENT_NO WEEK_PASSED,
	ul.EMPLOYEE_ID,ul.INVESTOR_ID,monthEndDate TRX_DATE,ul.MEMBER_CATEGORY_ID
 	FROM LOAN_SUMMARY ul 
 	INNER JOIN PRODUCTS p ON ul.PRODUCT_ID=p.PRODUCT_ID
	WHERE ul.OFFICE_ID=officeID AND ul.LOAN_STATUS=1
	AND ul.DISBURSE_DATE< vMonthStartDate
	AND p.PAYMENT_FREQUENCY='M'
	AND ul.INSTALLMENT_START_DATE<= monthEndDate
	AND ul.LOAN_SUMMARY_ID
	NOT IN
	(
	SELECT LOAN_SUMMARY_ID
	FROM LOAN_TRX ul 
	WHERE ul.OFFICE_ID=officeID AND ul.INSTALLMENT_DATE BETWEEN vMonthStartDate AND monthEndDate
	);
	...
	AND lr.INSTALLMENT_DATE=monthEndDate
	) s
	...
	t.INSTALLMENT_DATE = monthEndDate)
	WHEN MATCHED THEN
	...
	AND t.INSTALLMENT_DATE = monthEndDate)
	WHEN MATCHED THEN
